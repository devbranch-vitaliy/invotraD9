 ...
 *
 * @MigrateSource(
 *   id = "invotra_d7_webform",
 *   core = {7},
 *   source_module = "webform",
 *   destination_module = "webform"
 * )
 */
class InvotraD7Webform extends DrupalSqlBase implements ImportAwareInterface, RollbackAwareInterface {
  ...
  public function postImport(MigrateImportEvent $event) {
    ...
    $webforms = $this->prepareQuery()->execute();
    foreach ($webforms as $webformInfo) {
      $webform_nid = $webformInfo['nid'];
      $webform_id = $this->getWebformId($webform_nid, $webformInfo['machine_name']);
  ...
  public function postRollback(MigrateRollbackEvent $event) {
    ...
    foreach ($webforms as $webform) {
      ...
      $webform_id = $this->getWebformId($webform_nid, $webform['machine_name']);
      $webform = Webform::load($webform_id);
      if (empty($webform)) {
        /** @var \Drupal\node\NodeInterface $node */
        $node = Node::load($webform_nid);
